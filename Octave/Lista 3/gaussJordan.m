...
function gauss_jordan()
    ...
    fprintf('========================================\n');
    fprintf('GAUSS-JORDAN COM PIVOTAMENTO PARCIAL\n');
    fprintf('========================================\n\n');
    ...
    % Resolver o sistema
    [x, sucesso] = metodo_gauss_jordan(A, b);
...
function [x, sucesso] = metodo_gauss_jordan(A, b)
    % Cria a matriz aumentada
    Aa = [A b];
    n = size(A, 1);
    sucesso = true;

    % --- FASE 1: DIAGONALIZAÇÃO ---
    for i = 1:n
        % 1. Pivotamento Parcial (Integrado no loop - CORRETO)
        [~, max_idx] = max(abs(Aa(i:n, i)));
        max_idx = max_idx + i - 1;

        if max_idx ~= i
            Aa([i max_idx], :) = Aa([max_idx i], :);
        end

        % Verifica pivô nulo
        if abs(Aa(i, i)) < 1e-12
            sucesso = false; x = []; return;
        end

        % 2. Normalização da linha do pivô
        Aa(i, :) = Aa(i, :) / Aa(i, i);

        % 3. Eliminação da coluna (zera acima e abaixo)
        for j = 1:n
            if i ~= j
                fator = Aa(j, i);
